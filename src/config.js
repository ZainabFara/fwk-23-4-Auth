...
if (process.env.NODE_ENV === 'production') {
	if (!ACCESS_TOKEN_SECRET || !REFRESH_TOKEN_SECRET) {
		throw new Error("ACCESS_TOKEN_SECRET och REFRESH_TOKEN_SECRET krävs i produktionsmiljön för säkerhet.");
	}
}
...
const HTTP_ONLY = process.env.NODE_ENV === 'production' ? DEFAULT_HTTP_ONLY : process.env.HTTP_ONLY || false;


const AUTH = AUTH_TYPES[(process.env.AUTH || 'NONE').toUpperCase()] || AUTH_TYPES.NONE;
const SAME_SITE = process.env.NODE_ENV === 'production'
	? DEFAULT_SAME_SITE
	: SAME_SITE_TYPES[(process.env.SAME_SITE || 'NONE').toUpperCase()] || SAME_SITE_TYPES.NONE;
